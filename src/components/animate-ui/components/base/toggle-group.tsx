...
const [ToggleGroupProvider, useToggleGroup] = getStrictContext<VariantProps<typeof toggleVariants>>('ToggleGroupContext');
  ...
  return (
    ...
    >
      <ToggleGroupProvider value={{ variant, size }}>
        {!multiple ? <ToggleGroupHighlightPrimitive className='bg-accent rounded-md'>{children}</ToggleGroupHighlightPrimitive> : children}
      </ToggleGroupProvider>
    </ToggleGroupPrimitive>
  );
}
...
function Toggle({ className, children, variant, size, ...props }: ToggleProps) {
  ...
  return (
    <ToggleHighlightPrimitive value={props.value?.toString()} className={cn(multiple && 'bg-accent rounded-md')}>
      <TogglePrimitive
