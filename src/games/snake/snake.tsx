...
export function SnakeGame({ embedded = false }: { embedded?: boolean }) {
    ...
    <div className='flex items-start justify-center gap-8 w-full'>
      ...
      <div className='flex flex-col items-center justify-between gap-6 '>
        ...
        <div className='flex flex-col items-center gap-2'>
          ...
          <div className='flex items-center gap-2'>
            <span className='text-xs text-muted-foreground uppercase font-bold'>Start/Stop</span>
            <Kbd>{t('kbd.space')}</Kbd>
          </div>
        </div>
        ...
        {gameOver && (
          <div className='flex flex-col items-center gap-2 animate-in fade-in zoom-in duration-500'>
            ...
            <div className='text-sm text-muted-foreground animate-pulse'>{t('games.snake.restartHint')}</div>
          </div>
        )}
      </div>
      ...
      {highScores.length > 0 && (
        <div className='w-64 mt-2 shrink-0'>
          <div className='flex items-center justify-between mb-3'>
            <h3 className='text-sm font-bold uppercase tracking-wide'>{t('games.snake.leaderboard')}</h3>
            <Button variant='ghost' size='sm' onClick={handleClearLeaderboard} className='h-7 text-xs'>
              {t('games.snake.clear')}
            </Button>
          </div>
          <div className='space-y-1'>
            {highScores.map((score, index) => {
              ...
              const entry = (
                ...
                >
                  <span className={`text-sm font-medium ${medalClass}`}>#{index + 1}</span>
                  <span className={`font-mono font-bold tabular-nums ${medalClass}`}>{score.toString().padStart(3, '0')}</span>
                </div>
              );

              if (index === lastAddedScoreIndex) {
                return (
                  <ElectricBorder key={index} color='#7df9ff' speed={0.5} chaos={0.2} thickness={2} style={{ borderRadius: '0.375rem' }}>
                    {entry}
                  </ElectricBorder>
                );
              }
  ...
  const controls = (
    <div className='flex items-center gap-6'>
      <div className='flex flex-col items-end'>
        ...
        <span className='font-mono font-bold text-3xl tabular-nums leading-none'>{score.toString().padStart(3, '0')}</span>
      </div>
  ...
  return <GameLayout controls={controls}>{content}</GameLayout>;
}
