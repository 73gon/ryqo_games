  ...
  return (
    <div className='flex flex-col items-center gap-6 p-6 w-full max-w-md animate-in fade-in-0 duration-300'>
      ...
      <div className='text-center'>
        ...
        <div className='text-3xl font-mono font-bold text-primary tracking-wider'>{roomId}</div>
      </div>
          ...
          {players.map((player) => (
            <div
              ...
              className={`flex items-center justify-between p-3 rounded-md border ${
                player.id === currentPlayerId ? 'bg-primary/10 border-primary/30' : 'bg-muted/30 border-border'
              }`}
            >
              <span className='flex items-center gap-2'>
                <span className={`w-2 h-2 rounded-full ${player.id === currentPlayerId ? 'bg-primary' : 'bg-green-500'} animate-pulse`} />
                {player.name}
                {player.id === currentPlayerId && <span className='text-xs text-muted-foreground'>(You)</span>}
              </span>
              {isHost && player.id === currentPlayerId && <span className='text-xs bg-primary/20 text-primary px-2 py-0.5 rounded'>Host</span>}
            </div>
          ))}
        </div>
      </div>
      ...
      {isHost ? (
        ...
        <div className='text-center text-sm text-muted-foreground animate-pulse'>Waiting for host to start the race...</div>
      )}
    </div>
  );
});
