...
const Car = memo(function Car({ 
  ...
  return (
    <svg 
      ...
      height="16" 
      viewBox="0 0 32 16" 
      className={`drop-shadow-md ${isCurrentPlayer ? 'filter brightness-110' : ''}`}
    >
      {/* Car body */}
      ...
      {isCurrentPlayer && (
        <circle cx="16" cy="0" r="2" fill="#FFD700" className="animate-pulse" />
      )}
    </svg>
  );
});
          ...
          return (
            <div key={player.id} className="relative">
              ...
              <div className="h-10 bg-muted/40 rounded-md relative overflow-hidden border border-border/50">
                ...
                {/* Finish line */}
                <div className="absolute right-0 top-0 bottom-0 w-2 bg-gradient-to-b from-white via-black to-white opacity-50" 
              ...
              {/* Player info */}
              <div className={`flex items-center justify-between mt-1 text-xs ${isCurrentPlayer ? 'text-primary font-medium' : 'text-muted-foreground'}`}>
